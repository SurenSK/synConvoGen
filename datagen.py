...
model = AutoModelForCausalLM.from_pretrained("mistralai/Mistral-7B-Instruct-v0.2", load_in_8bit=True, device_map="auto", max_new_tokens=200, temperature=0.4)
model.eval()
llm = pipeline("text-generation", model=model, tokenizer=tokenizer, batch_size=batchSize)
...
def generate_questions(task, is_safe): 
    ...
    if is_safe:
        ...
        prompt += f"It is important that you never mention the task of {task} directly. Be circumspect. Do not let your counterparty become suspicious that you are engaged in the task of {task}."
